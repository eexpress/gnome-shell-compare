...
class Indicator extends PanelMenu.Button {
	_init() {
		...
		item0.file=''; item0.mime=''; item0.reactive = false;
		...
		item1.file=''; item1.mime=''; item1.reactive = false;
		...
		Main.wm.addKeybinding("open-last-file-in-termianl", ExtensionUtils.getSettings(), Meta.KeyBindingFlags.NONE, Shell.ActionMode.ALL, () => {
			if(item1.file) this.default_open(item1.file);
		...
		function judge(text, isPRIMARY){
			...
			//~ 当前目录是~，所以带./前缀的文件，都认为是~/的文件。
				...
				item0.file = item1.file; item0.mime = item1.mime;
				item0.reactive = item1.reactive;
				item1.file = text; item1.mime = contentType;
				item1.reactive = isPRIMARY;
				if(item0.mime == item1.mime) item.reactive = true;
				else item.reactive = false;
		...
		function markup(){
			for(let i=0; i<2; i++){	// 强制循环刷新
				...
				if(!a.file) {a.label.text = (i+1)+":"; a.reactive = false;}
				else {
